...
`ActionState`'s display values are now one word each -- `SHIPS_UNINSTALLED` reads `available`, `NO_PAGE` reads `missing`, `MANAGED` reads `managed` -- and `_STATE_COLOR` colours each (yellow/red/green respectively) so the State column scans by colour as well as text.
Enumerating $PATH and computing per-row state together can take several seconds on a real system (see Performance below), so `status()` wires `discovery.enumerate_installations`'s and `compute_status`'s new optional `on_start`/`on_scan` callbacks (mirroring `manpages.find_help_derived_manpages`'s existing convention) to a Rich progress bar spanning both phases, shown for the whole run rather than gated behind a delay -- deliberate, not a fallback: an always-visible bar was chosen over a delayed-reveal one after review.
The bare-name pipe path (`_bare_names`, `--names`) is untouched by any of this -- no progress bar, no colour, one binary per line via plain `print()`.

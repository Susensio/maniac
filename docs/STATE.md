...
`local_docs()` is now wired into `install` (Stage 6, below); `status` still treats every candidate as needing an LLM-generated page, which is Stage 7's job.
...
`list`, `list-missing`, `generate`/`generate-missing` and the standalone `compare` command are gone -- `generate` was renamed `install` by ADR-0016 (Stage 6, below), removed outright rather than kept as an alias.
...
Stage 6 landed: `generate` is renamed `install` (`maniac/cli/install.py`), the tier-selection logic lives in `maniac/orchestration/install.py`'s `run_install`, and `--generate`/`--no-generate` select tier 3 or tiers 1-2 respectively.
Tier order: `_try_install_root` (tier 1, `provider.local_docs(inst)` filtered to the one page naming the binary itself via `manpages.select_primary_manpage`, picking `pandoc.1.gz` over its `pandoc-lua`/`pandoc-server` siblings), then `_try_repository` (tier 2), then `run_pipeline` (tier 3, imported lazily inside `run_install` so `--no-generate` never even imports the LLM synthesis path, not merely leaves it uncalled).
`generate` is removed outright, not aliased -- matching ADR-0013's own precedent of dropping a renamed command rather than keeping a deprecated name live, and there being no external consumer of the CLI to break.

Tier 2 (`_try_repository`) refuses in two cases rather than guessing: `inst.version is None` (a raw `local_lib` checkout carries no installed version to match against) skips it outright, and `docs.resolve_repo_dir`'s new `version=` argument clones the git tag naming the installed version (`_find_matching_tag`, `git ls-remote --tags`, tried as `v<version>` then bare `<version>`) rather than the default branch, returning `None` -- not a same-named default-branch page -- when no tag matches.
A page that clears both is checked against the binary it claims to document via `manpages.manpage_documents`, matching its own `.TH`/`.Dt` title against the binary name; this does not prove the repository itself is the correct upstream (`docs/BACKLOG.md`'s `tmux`/`tmux-builds` case is a wrongly-resolved repository, not a wrongly-titled page), only that title and filename agree.

Folded in by ruling, closing the ADR-0015 gap `docs/BACKLOG.md` deferred from Stage 3: `discover_repo` no longer returns `RepoSource(name=binary, target=binary)` for a binary no provider claims -- it returns `None`, and `run_pipeline`/`cli/source.py`'s `docs` command handle that directly.
`PipelineResult.repo_source` widened to `RepoSource | None` to carry it; `discover_candidate_source` (`status`'s own resolution path) already returned `None` in this case before this stage; the fallback was effectively inert for `fetch_and_extract_docs` even before removal, since a `target == name` `RepoSource` has no `clone_url` either way, so removing it changes test assertions and `source docs`'s message but no synthesis outcome for a real binary.

Verified live: `maniac install pandoc --no-generate` prints `pandoc   upstream manpage from install root (3.10.2)   [no synthesis]` and copies `~/.local/share/mise/installs/pandoc/3.10.2/share/man/man1/pandoc.1.gz` verbatim to `~/.local/share/man/man1/pandoc.1.gz`; `man -w pandoc` and `man -l` against that path confirm it renders, completing Stage 5's `pandoc` acceptance case with no LLM call reached (`MANIAC_LLM_API_KEY=invalid` set for the run, no effect).
`tests/test_orchestration_install.py::test_no_generate_never_reaches_the_llm_when_no_tier_1_or_2_page_exists` covers the same guarantee against a monkeypatched `run_llm_synthesis` that raises if called, with both tiers failing (not the happy path) -- the case a structural regression would actually break.

A real defect surfaced only by the live run, not the unit tests: `_render_install`'s detail string carries a literal `[no synthesis]`, which Rich's markup parser reads as an (invalid, silently dropped) style tag -- the live `--no-generate` run printed the tool and tier with the bracketed suffix missing entirely.

...
`list` reports six states by checking manpage reachability: *ok* (a page resolves and local evidence says it is current), *unverified* (an external page resolves but its package/version match cannot be proven), *misattributed* (positive evidence says a page belongs to a different package entirely), *outdated* (positive evidence says a page documents another version), *available* (nothing resolves, but a page can be had without LLM synthesis), *missing* (nothing resolves and no free page is known). Use `--unverified`, `--misattributed`, `--outdated`, `--available`, or `--missing` to select a state.
